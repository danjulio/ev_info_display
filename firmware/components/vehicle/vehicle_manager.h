...
typedef struct {
	...
	int req_len;                // Number of valid bytes in the request (1-8)
	uint8_t data[8];            // CAN Data
	bool is_indexed;            // Set for indexed requestes (e.g. individual cell voltages)
	int index_loc;              // data[] byte (0-7) containing the base index value
} can_request_t;
...
typedef struct {
	...
	item_range_t hv_batt_cell_range;
	bool can_is_500k;
...
// For vehicle implementation, data broker and GUI use
