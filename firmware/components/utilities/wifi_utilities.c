	...
	if (init_esp_wifi()) {
		...
		if (configP->sta_mode) {
			...
			if (enable_esp_wifi_ap()) {
				wifi_flags |= WIFI_INFO_FLAG_ENABLED;
				ESP_LOGI(TAG, "WiFi AP %s enabled", configP->ap_ssid);
			} else {
				return false;
			}
		}
...
bool wifi_reinit()
{
	...
	ps_get_config(PS_CONFIG_TYPE_NET, (void**) &configP);
...
void wifi_get_ipv4_addr_string(char* s)
{
...
void wifi_get_ipv4_gw_string(char* s)
{
	esp_netif_ip_info_t ip_info;
	
	esp_netif_get_ip_info(wifi_netif, &ip_info);
	sprintf(s, IPSTR, IP2STR(&ip_info.gw));
}
...
static bool enable_esp_wifi_ap()
{
	...
	wifi_config_t wifi_driver_config = {
        .ap = {
            .ssid_len = strlen(configP->ap_ssid),
            .max_connection = 1,
            .authmode = WIFI_AUTH_WPA_WPA2_PSK
        }
    };
    strcpy((char*) wifi_driver_config.ap.ssid, configP->ap_ssid);
    strcpy((char*) wifi_driver_config.ap.password, configP->ap_pw);
    if (strlen(configP->ap_pw) == 0) {
...
static bool enable_esp_wifi_client()
{
	...
	if (configP->sta_static_ip) {
		...
		ipInfo.ip.addr = configP->sta_ip_addr[3] |
						 (configP->sta_ip_addr[2] << 8) |
						 (configP->sta_ip_addr[1] << 16) |
						 (configP->sta_ip_addr[0] << 24);
		ipInfo.gw.addr = esp_netif_ip4_makeu32(0, 0, 0, 0);
  		ipInfo.netmask.addr = configP->sta_netmask[3] |
						     (configP->sta_netmask[2] << 8) |
						     (configP->sta_netmask[1] << 16) |
						     (configP->sta_netmask[0] << 24);
    ...
    strcpy((char*) wifi_driver_config.sta.ssid, configP->sta_ssid);
    if (strlen(configP->sta_pw) == 0) {
        strcpy((char*) wifi_driver_config.sta.password, "");
    } else {
    	strcpy((char*) wifi_driver_config.sta.password, configP->sta_pw);
    }
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
	...
	switch (event_id) {
		...
		case WIFI_EVENT_STA_START:
			ESP_LOGI(TAG, "Station started, trying to connect to %s", configP->sta_ssid);
			esp_wifi_connect();
			sta_retry_num = 0;
        	break;
        ...
        case WIFI_EVENT_STA_DISCONNECTED:
            ...
            esp_wifi_connect();
            ESP_LOGI(TAG, "Retry connection to %s", configP->sta_ssid);
        	break;
	}
}
...
static void ip_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
	...
	ESP_LOGI(TAG, "Gateway IP Address: " IPSTR, IP2STR(&ip_info->ip));
}
